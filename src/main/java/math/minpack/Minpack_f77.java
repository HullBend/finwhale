...
// the scaled directional derivative.
                ...
                temp2 = (FastMath.sqrt(par[1]) * pnorm) / fnorm;
...
// Calculation of norm.
        ...
        if (s1 != zero) {
            ...
            enorm = x1max * FastMath.sqrt(s1 + (s2 / x1max) / x1max);
            ...
            if (s2 != zero) {
                ...
                if (s2 >= x3max) {
                    ...
                    enorm = FastMath.sqrt(s2 * (one + (x3max / s2) * (x3max * s3)));

                } else {

                    enorm = FastMath.sqrt(x3max * ((s2 / x3max) + (x3max * s3)));

                }

            } else {

                enorm = x3max * FastMath.sqrt(s3);
                ...
                if (n >= jp1) {
                    ...
                    for (k = jp1; k <= n; k++) {
                        ...
                        if (pivot && rdiag[k] != zero) {
                            ...
                            temp = a[j][k] / rdiag[k];
                            rdiag[k] *= FastMath.sqrt(Math.max(zero, one - temp * temp));
...
// appropriate element in the current row of D.
                    ...
                    if (sdiag[k] != zero) {
                        ...
                        if (Math.abs(r[k][k]) < Math.abs(sdiag[k])) {
                            ...
                            cotan = r[k][k] / sdiag[k];
                            sin = p5 / FastMath.sqrt(p25 + p25 * cotan * cotan);
                            cos = sin * cotan;

                        } else {

                            tan = sdiag[k] / r[k][k];
                            cos = p5 / FastMath.sqrt(p25 + p25 * tan * tan);
                            sin = cos * tan;
...
*/
        ...
        eps = FastMath.sqrt(Math.max(epsfcn, epsmch));
