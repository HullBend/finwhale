 ...
 */
final class AndersonDarling {

    /**
     * Computes the <b>complementary</b> distribution function
     * bar(F)<sub>n</sub>(<em>x</em>) with parameter {@code n}.
     */
    static double barF(int n, double x) {
        ...
        if (x > 5.0) {
            ...
            double nd = n;
            q = (0.23945 * FastMath.pow(nd, -0.9379) - 0.1201 * FastMath.pow(nd, -0.96) - 1.0002816) * x
                    - 1.437 * FastMath.pow(nd, -0.9379) + 1.441 * FastMath.pow(nd, -0.96) - 0.0633101;
            return FastMath.pow(x, -0.48897) * FastMath.exp(q);
        }
    ...
    private static double cdf_N_1(double x) {
        ...
        return Math.sqrt(1.0 - 4.0 * FastMath.exp(-x - 1.0));
    }
    ...
    private static double barF_N_1(double x) {
        ...
        double q;
        if (x < 6.0) {
            q = 1.0 - 4.0 * FastMath.exp(-x - 1.0);
            return 1.0 - Math.sqrt(q);
        }
        q = 4.0 * FastMath.exp(-x - 1.0);
    ...
    private static double lower_Marsaglia(int n, double x) {
        ...
        p0 *= FastMath.exp(-1.2337141 / x) / Math.sqrt(x);
