...
final class KolmogorovSmirnovP {
    ...
    static double cdf(int n, double x) {
        ...
        if (n * x <= 6.5) {
            ...
            for (int j = 1; j <= jmax; j++) {
                ...
                if (-q > Double.MIN_NORMAL) {
                    term = logCom + jreal * Math.log(-q) + (njreal - 1.0) * FastMath.log1p(-q);
                    sum += sign * FastMath.exp(term);
                }
                sign = -sign;
                logCom += Math.log(njreal / (j + 1));
            }
            // add the term j = 0
            sum += FastMath.exp((n - 1) * FastMath.log1p(x));
            return sum * x;
        }
        ...
        if (n <= 4000) {
            ...
            for (int j = 1; j <= jmax; j++) {
                ...
                double njreal = n - j;
                q = jreal / n + x;
                term = logCom + (jreal - 1.0) * Math.log(q) + njreal * FastMath.log1p(-q);
                sum += FastMath.exp(term);
            ...
            if (1.0 > x) {
                sum += FastMath.exp(n * FastMath.log1p(-x));
            }
            return 1.0 - sum;
        }
    ...
    static double kolmoSmirnovPlusBarUpper(int n, double x) {
        ...
        while (j <= jmax) {
            ...
            double term = logCom + (j - 1) * Math.log(q) + (n - j) * FastMath.log1p(-q);
            double t = FastMath.exp(term);
            sum += t;
