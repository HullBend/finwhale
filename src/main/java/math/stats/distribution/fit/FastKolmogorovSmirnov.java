...
final class FastKolmogorovSmirnov {
    ...
    private static double barFConnu(int n, double x) {
        ...
        if (x <= 1.0 / n) {
            ...
            if (n <= NEXACT) {
                ...
                return 1.0 - v * FastMath.pow(t, (double) n);
            }
            double v = Arithmetic.logFactorial(n) + n * Math.log(t / n);
            return 1.0 - FastMath.exp(v);
        }

        if (x >= 1.0 - 1.0 / n) {
            return 2.0 * FastMath.pow(1.0 - x, (double) n);
        }
    ...
    private static double cdfConnu(int n, double x) {
        ...
        if (x <= 1.0 / n) {
            ...
            if (n <= NEXACT) {
                ...
                return w * FastMath.pow(t, (double) n);
            }
            double w = Arithmetic.logFactorial(n) + n * Math.log(t / n);
            return FastMath.exp(w);
        }

        if (x >= 1.0 - 1.0 / n) {
            return 1.0 - 2.0 * FastMath.pow(1.0 - x, (double) n);
        }
    ...
    private static double pomeranz(int n, double x) {
        ...
        final double RENO = FastMath.scalb(1.0, ENO);
        final double t = n * x;
    ...
    private static void calcFloorCeil(int n /* sample size */,
        ...
        final double w = FastMath.ceil(t) - t;
    ...
    private static double durbinMatrix(int n, double d) {
        ...
        for (int i = 0; i < m; i++) {
            H[i * m] -= FastMath.pow(h, (double) (i + 1));
            H[(m - 1) * m + i] -= FastMath.pow(h, (double) (m - i));
        }
        H[(m - 1) * m] += (2.0 * h - 1.0 > 0.0 ? FastMath.pow(2.0 * h - 1.0, (double) m) : 0.0);
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < m; j++) {
                if (i - j + 1 > 0) {
        ...
        s *= FastMath.pow(10.0, (double) pQ[0]);
        return s;
    }
    ...
    private static double pelz(int n, double x) {
        ...
        while (j <= JMAX && FastMath.abs(term) > EPS * FastMath.abs(tom)) {
            double ti = j + 0.5;
            term = (PI2 * ti * ti - z2) * FastMath.exp(-ti * ti * w);
            tom += term;
            j++;
        }
        ...
        while (j <= JMAX && term > EPS * tom) {
            double ti = j;
            term = PI2 * ti * ti * FastMath.exp(-ti * ti * w);
            tom += term;
            j++;
        }
